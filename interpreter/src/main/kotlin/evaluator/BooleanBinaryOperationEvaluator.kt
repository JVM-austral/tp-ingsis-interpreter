...
class BooleanBinaryOperationEvaluator(
    private val engine: AstEvaluator,
    private val strategies: List<BooleanOperationStrategy>,
) : AstEvaluator {
    override fun evaluate(
        ...
        for (strategy in strategies) {
            if (strategy.canHandle(ast.getValue())) {
                if (leftValue.javaClass !=
                    rightValue.javaClass
                ) {
                    throw Exception("Type mismatch: cannot compare $leftValue with $rightValue")
                }
