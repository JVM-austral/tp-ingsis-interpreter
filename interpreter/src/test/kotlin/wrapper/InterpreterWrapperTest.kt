...
class InterpreterWrapperTest {
    ...
    private class AstIteratorWrapper(
        private val asts: List<Result<Ast>>,
    ...
    fun `Var declaration with bool op assigment, should assign a bool value via wrapper`() {
        val interpreter =
            InterpreterFactory().createInterpreterV2(
                heap,
                mock.StdOutputHandler(),
                mutableMapOf(),
                evaluator.input.MockInputProvider("hola"),
                evaluator.input.LiteralConverter(),
            )
        val assigment =
            VarDeclaration(
                "let",
    ...
    fun `Var declaration with bool op assigment, should fail with illegal arguments via wrapper`() {
        val analyzer =
            analyzer.VarDeclarationWithAssigmentBinaryAnalyzer(
                factory.evaluators.AstEvaluationEngineV2(
                    mock.StdOutputHandler(),
                    evaluator.input.MockInputProvider("hola"),
                    evaluator.input.LiteralConverter(),
                ),
