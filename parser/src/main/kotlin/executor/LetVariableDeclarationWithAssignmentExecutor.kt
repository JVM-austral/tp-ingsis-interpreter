...
class LetVariableDeclarationWithAssignmentExecutor(private val operatorAnalyzers: List<StructureAnalyzer>) : StructureExecutor {
    override fun execute(tokens: List<Token>): Ast {
        ...
        return VarDeclaration(tokens[0].value, StringLiteral(tokens[1].value, tokens[1].line, tokens[1].column), TypeDeclaration(tokens[3].value, tokens[3].line, tokens[3].column), StringLiteral(tokens[tokens.size - 2].value, tokens[tokens.size - 2].line, tokens[tokens.size - 2].column), tokens[0].line, tokens[0].column)
    }
}
