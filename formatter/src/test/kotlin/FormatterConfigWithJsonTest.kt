...
class FormatterConfigWithJsonTest {
    ...
    @Test
    fun `test buildAnalyzers with custom config file v 1`() {
        val formatter = ConfigurableAnalyzerFormatter("src/test/resources/exampleconfig1.json", 1)
    ...
    @Test
    fun `test buildAnalyzers with custom config file v 2`() {
        val formatter = ConfigurableAnalyzerFormatter("src/test/resources/exampleconfig2.json", 2)
        val analyzers = formatter.buildFormatter().getAnalyzers()
        ...
        assertTrue(analyzers.any { it is IfOpenBlockInTheSameLineAnalyzer })
    }
}
